...
const hasUsablePolygons = (points: Vec2[][]): boolean =>
	points.some((polygon) => polygon.length >= 3);
...
export const makeManifoldFrustum = (
	...
	let manifold = CrossSection.square([baseWidth, baseLength], true).extrude(
		height,
		0,
		0,
		[topWidthScale, topLengthScale],
		true,
	);

	if (hasUsablePolygons(name.points)) {
		...
		manifold = manifold[operation](nameExtrusion.translate(nameSlotPosition));
	}

	if (hasUsablePolygons(year.points)) {
		...
		manifold = manifold[operation](yearExtrusion.translate(yearSlotPosition));
	}
